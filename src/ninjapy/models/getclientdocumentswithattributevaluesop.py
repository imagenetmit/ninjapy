"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from enum import Enum
from ninjapy.types import BaseModel
from ninjapy.utils import FieldMetadata, QueryParamMetadata
import pydantic
from typing import List, Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class GroupBy(str, Enum):
    TEMPLATE = "TEMPLATE"
    ORGANIZATION = "ORGANIZATION"


class GetClientDocumentsWithAttributeValuesRequestTypedDict(TypedDict):
    group_by: NotRequired[GroupBy]
    organization_ids: NotRequired[str]
    template_ids: NotRequired[str]
    template_name: NotRequired[str]
    document_name: NotRequired[str]


class GetClientDocumentsWithAttributeValuesRequest(BaseModel):
    group_by: Annotated[
        Optional[GroupBy],
        pydantic.Field(alias="groupBy"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None

    organization_ids: Annotated[
        Optional[str],
        pydantic.Field(alias="organizationIds"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None

    template_ids: Annotated[
        Optional[str],
        pydantic.Field(alias="templateIds"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None

    template_name: Annotated[
        Optional[str],
        pydantic.Field(alias="templateName"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None

    document_name: Annotated[
        Optional[str],
        pydantic.Field(alias="documentName"),
        FieldMetadata(query=QueryParamMetadata(style="form", explode=True)),
    ] = None


class GetClientDocumentsWithAttributeValuesValueTypedDict(TypedDict):
    r"""Value"""


class GetClientDocumentsWithAttributeValuesValue(BaseModel):
    r"""Value"""


class GetClientDocumentsWithAttributeValuesFieldsTypedDict(TypedDict):
    r"""Updated Fields"""

    name: NotRequired[str]
    r"""Name"""
    value: NotRequired[GetClientDocumentsWithAttributeValuesValueTypedDict]
    r"""Value"""
    value_update_time: NotRequired[float]
    r"""Value Last Updated"""


class GetClientDocumentsWithAttributeValuesFields(BaseModel):
    r"""Updated Fields"""

    name: Optional[str] = None
    r"""Name"""

    value: Optional[GetClientDocumentsWithAttributeValuesValue] = None
    r"""Value"""

    value_update_time: Annotated[
        Optional[float], pydantic.Field(alias="valueUpdateTime")
    ] = None
    r"""Value Last Updated"""


class GetClientDocumentsWithAttributeValuesResponseBodyTypedDict(TypedDict):
    document_id: NotRequired[int]
    r"""Document Identifier"""
    document_name: NotRequired[str]
    r"""Document Name"""
    document_description: NotRequired[str]
    r"""Document Description"""
    document_update_time: NotRequired[float]
    r"""Document Last Updated"""
    fields: NotRequired[List[GetClientDocumentsWithAttributeValuesFieldsTypedDict]]
    r"""Fields"""
    document_template_id: NotRequired[int]
    r"""Document Template Identifier"""
    document_template_name: NotRequired[str]
    r"""Document Template Name"""
    organization_id: NotRequired[int]
    r"""Organization Identifier"""


class GetClientDocumentsWithAttributeValuesResponseBody(BaseModel):
    document_id: Annotated[Optional[int], pydantic.Field(alias="documentId")] = None
    r"""Document Identifier"""

    document_name: Annotated[Optional[str], pydantic.Field(alias="documentName")] = None
    r"""Document Name"""

    document_description: Annotated[
        Optional[str], pydantic.Field(alias="documentDescription")
    ] = None
    r"""Document Description"""

    document_update_time: Annotated[
        Optional[float], pydantic.Field(alias="documentUpdateTime")
    ] = None
    r"""Document Last Updated"""

    fields: Optional[List[GetClientDocumentsWithAttributeValuesFields]] = None
    r"""Fields"""

    document_template_id: Annotated[
        Optional[int], pydantic.Field(alias="documentTemplateId")
    ] = None
    r"""Document Template Identifier"""

    document_template_name: Annotated[
        Optional[str], pydantic.Field(alias="documentTemplateName")
    ] = None
    r"""Document Template Name"""

    organization_id: Annotated[
        Optional[int], pydantic.Field(alias="organizationId")
    ] = None
    r"""Organization Identifier"""
